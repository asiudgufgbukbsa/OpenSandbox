...
volumes:
  - name: workdir
    ossfs:
      ...
      accessKeyId: "AKIDEXAMPLE"
      accessKeySecret: "SECRETEXAMPLE"
      securityToken: "STS_TOKEN_EXAMPLE"
      version: "2.0"
      options:
        - "allow_other"
        - "umask=0022"
    ...
    volumes=[
        Volume(
            ...
            ossfs=OSSFS(
                ...
                security_token="STS_TOKEN_EXAMPLE",
                version="2.0",
...
- For `ossfs` backend, validate required fields (`bucket`, `endpoint`, `accessKeyId`, `accessKeySecret`) and optional `securityToken`.
...
This change is additive for volume support and supports OSSFS inline credentials (including temporary STS token). If a client submits volume fields to a runtime that does not support them, the API will return a clear validation error.
